...
if os.path.exists("temp_art.png"):
    ...
    if st.session_state.is_locked:
        ...
        <div style="font-family: system-ui, -apple-system, sans-serif; text-align: center;">
            <a id="step1-btn" href=""" + ADSTERRA_DIRECT_LINK + """ target="_blank" onclick="unlockStep2()" 
            ...
            <a id="step2-btn" target="_parent" 
               style="display: block; width: 100%; background: #334155; color: #64748b; font-weight: 800; font-size: 1.1rem; padding: 1rem; border-radius: 12px; text-decoration: none; pointer-events: none; transition: all 0.2s ease; box-sizing: border-box;">
        ...
        components.html(ad_html_code, height=160)
...
if os.path.exists("temp_art.png"):
    ...
    if st.session_state.is_locked:
        ...
        # Safe HTML block using string concatenation (no brace conflicts)
        ad_html_code = """
        <div style="font-family: system-ui, -apple-system, sans-serif; text-align: center;">
            <a id="step1-btn" href=""" + ADSTERRA_DIRECT_LINK + """ target="_blank" onclick="unlockStep2()" 
               style="display: block; width: 100%; background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: #ffffff; font-weight: 800; font-size: 1.1rem; padding: 1rem; border-radius: 12px; text-decoration: none; box-shadow: 0 8px 20px rgba(34, 197, 94, 0.35); margin-bottom: 0.8rem; box-sizing: border-box;">
...
if os.path.exists("temp_art.png"):
    ...
    if st.session_state.is_locked:
        ...
        # Safe HTML block using .format() instead of f-strings to prevent brace errors
        ad_html_template = """
        <div style="font-family: system-ui, -apple-system, sans-serif; text-align: center;">
            <a id="step1-btn" href="{ad_link}" target="_blank" onclick="unlockStep2()" 
               style="display: block; width: 100%; background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: #ffffff; font-weight: 800; font-size: 1.1rem; padding: 1rem; border-radius: 12px; text-decoration: none; box-shadow: 0 8px 20px rgba(34, 197, 94, 0.35); margin-bottom: 0.8rem; box-sizing: border-box;">
                1️⃣ Tap Here to Visit Sponsor Link 🔗
            </a>
        ...
        components.html(ad_html_template.format(ad_link=ADSTERRA_DIRECT_LINK), height=160)
...
if os.path.exists("temp_art.png"):
    ...
    if st.session_state.is_locked:
        ...
        # Enforced Lock (Mobile Safe via Native HTML anchor tags)
        components.html(
            f"""
            <div style="font-family: system-ui, -apple-system, sans-serif; text-align: center;">
                <a id="step1-btn" href="{ADSTERRA_DIRECT_LINK}" target="_blank" onclick="unlockStep2()" 
            ...
            <script>
                function unlockStep2() {{
                    var btn2 = document.getElementById('step2-btn');
                    // Give it the reload URL
                    btn2.href = '?unlocked=true';
                    // Make it physically clickable
                    btn2.style.pointerEvents = 'auto';
                    btn2.style.background = 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)';
                    btn2.style.color = '#ffffff';
                    btn2.innerHTML = '2️⃣ Tap to Reveal Artwork ✨';
